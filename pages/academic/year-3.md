...
I computed posteriors for progressively fewer features (three, two, then one) and discovered a **non-monotonic progression**: the posterior $P(U=1)$ went from 0.36 (three features) to 0.6 (two features) to 0.5 (one feature), fluctuating rather than converging smoothly. This happens because each feature provides conflicting evidence about the class label. I produced a TikZ diagram illustrating this behaviour and cited Domingos and Pazzani (1997) who mathematically demonstrated that Naive Bayes can still be optimal for classification even when independence is violated, as long as the class ranking of posteriors is preserved.
